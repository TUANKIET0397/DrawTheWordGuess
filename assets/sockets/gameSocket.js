...
module.exports = (io) => {
    ...
    let currentWord = ""
    let hintUsage = { count: 0, drawerId: null } // Lưu số lần gợi ý đã dùng cho mỗi lượt
    ...
    io.on("connection", (socket) => {
        ...
        socket.on("joinGame", async (playerName) => {
            ...
            if (players.length >= 3 && !gameStarted) {
                gameStarted = true
                startTurn() // Bắt đầu lượt đầu tiên khi có đủ 3 người
        ...
        socket.on("requestHint", async (data) => {
            ...
            if (hintUsage.count >= 3) {
                io.to(socket.id).emit("hintLimitReached", {
                    message: "Bạn đã dùng hết gợi ý.",
                })
                return
            }

            const level = Math.min(
                Math.max(parseInt(data?.hintLevel) || hintUsage.count + 1, 1),
                3
            )
            ...
            const generateServerHint = (word, level) => {
                ...
                if (level === 1) {
                    return `This word has ${len} letters and starts with the letter "${w
                        .charAt(0)
                        .toUpperCase()}"`
                }
                if (level === 2) {
                    return `This word starts with the letter "${w
                        .charAt(0)
                        .toUpperCase()}" and ends with the letter "${w
                        .charAt(len - 1)
                        .toUpperCase()}"`
                }
    ...
    async function nextTurn() {
        ...
        if (hasCorrectGuess && currentDrawer) {
            ...
            const hintPenalty =
                hintUsage.drawerId === currentDrawer.id
                    ? hintUsage.count || 0
                    : 0
